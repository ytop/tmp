...
class SingleRoIExtractor(nn.Cell):
    ...
    def construct(self, rois, feat1, feat2, feat3, feat4):
        ...
        for i in range(self.num_levels):
            # x1 = P.Sub()(target_lvls, P.ScalarToArray()(i))
            # x2 = P.Cast()(x1, mstype.float32)
            # x3 = P.Abs()(x2)
            # x_p = P.ZerosLike()(x3)
            # x4 = P.Sub()(x_p, x3)
            # x5 = P.Argmax()(x4)
            # rois_tmp = P.GatherV2()(rois, x5, 0)
            # roi_feats_t = self.roi_layers[i](feats[i], rois_tmp)
            mask = self.equal(target_lvls, P.ScalarToArray()(i))
            mask_rois = P.Reshape()(mask, (-1, 1))
            mask_rois = self.cast(P.Tile()(self.cast(mask_rois, mstype.int32), (1, 5)), mstype.bool_)
            zero_rois = P.ZerosLike()(rois)
            rois_tmp = self.select(mask_rois, rois, zero_rois)
            roi_feats_t = self.roi_layers[i](feats[i], rois_tmp)
